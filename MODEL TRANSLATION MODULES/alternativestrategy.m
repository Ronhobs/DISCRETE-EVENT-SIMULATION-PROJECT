...
while ~isempty(FEL.list) 
    ...
    if isempty(nextEvent)
        ...
        simulationClock = nextEvent.time;
            % Verbose output for event processing
        if verbose
            disp(['Processing event of type ', char(nextEvent.type), ' at time ', num2str(simulationClock)]);
        end
    ...
    if simulationClock >= minAvailableTime
        ...
        switch nextEvent.type
            case EventType.CommuterArrivals
                    if verbose
                        disp(['Commuter Arrival at Time: ', num2str(simulationClock)]);
                    end
            ...
            case EventType.CommuterBags
                    if verbose
                        disp(['Processing Commuter Bags at Time: ', num2str(simulationClock)]);
                    end
              ...
              case EventType.ServiceTimeBoardingPass
                    if verbose
                        disp(['Printing Boarding Pass at Time: ', num2str(simulationClock)]);
                    end
    ...
    for serverIdx = 1:numServers
        if ~isempty(queueEntryTimes{serverID})
            ...
            case EventType.ServiceTimeCheckingBags
                    if verbose
                        disp(['Checking Bags at Time: ', num2str(simulationClock)]);
                    end
            ...
            case EventType.ServiceTimeProblemsandDelays
                    if verbose
                        disp(['Resolving Problems and Delays at Time: ', num2str(simulationClock)]);
                    end
            ...
            case EventType.ServiceTimeScreeningMachine
                    if verbose
                        disp(['Screening at Time: ', num2str(simulationClock)]);
                    end
                    % ... 
